...
func (customT *Translator) Translate(args *tt_translator.TranslateArgs) (*tt_translator.TranslateRes, error) {
	...
	if err := json.Unmarshal(respBytes, newResp); err != nil {
		tt_log.GetInstance().Error(fmt.Sprintf("引擎: %s, 错误: 解析报文异常(%s)", customT.GetName(), err))
		return nil, fmt.Errorf("错误: 解析报文出现异常(%s)", err.Error())
	}

	if newResp.ErrorCode != "0" {
		errMsg := errorMap[newResp.ErrorCode]
		tt_log.GetInstance().Error(fmt.Sprintf("引擎: %s, 错误: 接口响应异常(%s:%s)", customT.GetName(), newResp.ErrorCode, errMsg))

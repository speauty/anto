...
func (customT *Translator) IsValid() bool {
	return customT.isClientOk && customT.cfg.GetAK() != "" && customT.cfg.GetSK() != "" && customT.cfg.(*Cfg).GetRegion() != ""
}
...
func (customT *Translator) Translate(ctx context.Context, args *translator.TranslateArgs) (*translator.TranslateRes, error) {
	...
	req := &remoteReq{
		...
		Target:      &args.ToLang,
		ProjectId:   &customT.cfg.(*Cfg).ProjectId,
	}
...
func (customT *Translator) clientBuilder() {
	if customT.cfg.GetAK() == "" || customT.cfg.GetSK() == "" || customT.cfg.(*Cfg).GetRegion() == "" {
		return
	}
	tmpClient, tmpErr := common.NewClientWithSecretId(customT.cfg.GetAK(), customT.cfg.GetSK(), customT.cfg.(*Cfg).GetRegion())
	if tmpErr != nil {
